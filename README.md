...
| Secret | Value |
| --- | --- |
| `TRANSCRIPT_URL` | Your full browser transcript URL if the stable default URL does not load transcript content from GitHub Actions |
...
That `v` value is generated by the portal and can change. The monitor handles it in two separate ways:

- for fetching, it first tries the URL exactly as configured
- for hashing, email text, and saved state, it removes the generated `v` value so a changing URL token does not create false grade-change emails
...
If a manual `send_current=true` test fails because no transcript body is found, save the full browser URL as the `TRANSCRIPT_URL` repository secret and run the manual snapshot test again. If that generated URL later expires, the monitor will fail loudly and send a failure email instead of silently sending an empty snapshot.
